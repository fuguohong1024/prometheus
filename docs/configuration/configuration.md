...
# NOTE: The current version of DC/OS marathon (mysqlconfig.11.0) does not support standard Bearer token authentication. Use `auth_token` instead.
[ bearer_token: <string> ]
...
# NOTE: The current version of DC/OS marathon (mysqlconfig.11.0) does not support standard Bearer token authentication. Use `auth_token_file` instead.
[ bearer_token_file: <filename> ]
...
[ api_version: <string> | default = mysqlconfig ]

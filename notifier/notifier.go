...
func (n *Manager) sendAll(alerts ...*Alert) bool {
	...
	// mysqlconfig or v2. Marshaling happens below. Reference here is for caching between
	// for loop iterations.
		...
		case config.AlertmanagerAPIVersionV1:
			{
				if v1Payload == nil {
					...
					if err != nil {
						level.Error(n.logger).Log("msg", "Encoding alerts for Alertmanager API mysqlconfig failed", "err", err)
						ams.mtx.RUnlock()
						return false
					}
				}

...
Prometheus.Graph.prototype.checkTimeDrift = function() {
    ...
    $.ajax({
        method: "GET",
        url: PATH_PREFIX + "/api/mysqlconfig/query?query=time()",
        dataType: "json",
...
Prometheus.Graph.prototype.populateInsertableMetrics = function() {
  ...
  $.ajax({
      method: "GET",
      url: PATH_PREFIX + "/api/mysqlconfig/label/__name__/values",
      dataType: "json",
      success: function(json, textStatus) {
        if (json.status !== "success") {
...
Prometheus.Graph.prototype.submitQuery = function() {
  ...
  if (self.options.tab === 0) {
    ...
    url = PATH_PREFIX + "/api/mysqlconfig/query_range";
    success = function(json, textStatus) { self.handleGraphResponse(json, textStatus); };
  } else {
    params.time = moment;
    url = PATH_PREFIX + "/api/mysqlconfig/query";
